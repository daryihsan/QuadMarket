...
<span class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full">{{ $pending_verifications_count }}</span>
                    @endif
                </a>
...
<p class="text-3xl font-bold text-gray-800">{{ number_format($total_penjual_aktif) }}</p>
                    </div>
                </div>
...
<p class="text-3xl font-bold text-gray-800">{{ number_format($total_penjual_tidak_aktif) }}</p>
                    </div>
                </div>
...
<p class="text-3xl font-bold text-gray-800">{{ number_format($total_commenters) }}</p>
                    </div>
                </div>
...
<p class="text-3xl font-bold text-gray-800">{{ number_format($total_penjual) }}</p>
                    </div>
                </div>
            </div>
...
<p class="text-2xl font-bold text-blue-900">{{ number_format($total_penjual) }}</p>
...
<div class="space-y-2 pt-4 border-t">
                        @forelse($location_chart_data as $index => $item)
                            
<div class="flex justify-between items-center text-sm">
                                
<div class="flex items-center">
                                    
<span class="inline-block w-3 h-3 rounded-full mr-2" style="background-color: {{ $location_colors[$index % count($location_colors)] }}"></span>
                                    {{ $item['provinsi'] }}
                                </div>
...
<script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // =====================================================
            const productLabels = @json($product_labels);
            const productCounts = @json($product_counts);
            ...
            // =====================================================
            const locationLabels = @json($location_labels);
            const locationPercentages = @json($location_percentages);
            const locationColors = @json($location_colors);
            const locationCtx = document.getElementById('locationDonutChart');
            const totalSellers = {{ $total_penjual }};
