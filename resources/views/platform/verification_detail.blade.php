...
<body class="bg-gray-200">
    <div class="flex min-h-screen">
        <aside class="w-64 bg-white shadow-lg p-6">
            ...
            <a href="{{ route('platform.dashboard') }}" class="block p-2 text-gray-600 hover:bg-gray-100 rounded-lg">Dashboard</a>
            <a href="{{ route('platform.verifikasi.list') }}" class="block p-2 text-white blue-gradient rounded-lg font-semibold">Verifikasi Penjual</a>
            <a href="{{ route('platform.laporan') }}" class="block p-2 text-gray-600 hover:bg-gray-100 rounded-lg">Laporan</a>
        </aside>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                    <div>
                        ...
                        <p class="text-lg text-gray-900 mb-3">
                            {{ $seller->nama_jalan_pic }}, RT {{ $seller->rt }} / RW {{ $seller->rw }}, Kel. {{ $seller->nama_kelurahan }}, Kab/Kota {{ $seller->kabupaten_kota }}, **Propinsi {{ $seller->propinsi }}** [cite: 37, 38, 39, 40, 41, 42]
                        </p>
                    </div>
                ...
                <div class="mt-6 pt-4 border-t">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <a href="{{ $seller->foto_pic_url }}" target="_blank" class="text-blue-600 hover:underline">Lihat Foto PIC</a>
                            </div>
                        <div>
                            <p class="text-sm font-medium text-gray-500">14. File Upload KTP PIC [cite: 45]</p>
                            <a href="{{ $seller->file_upload_ktp_url }}" target="_blank" class="text-blue-600 hover:underline">Lihat File KTP PIC (.pdf/.jpg)</a>
                            </div>
                    </div>
                </div>
            </div>
