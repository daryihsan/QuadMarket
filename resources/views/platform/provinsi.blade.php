    ...
    <main class="main-content">
        ...
        <div class="bg-white rounded-t-lg shadow">
            <div class="flex border-b">
                <a href="{{ route('platform.laporan') }}"
                   class="px-6 py-4 text-gray-600 hover:text-gray-800 hover:bg-gray-50 whitespace-nowrap">
                    Daftar Penjual
                </a>

                <a href="{{ route('platform.laporan.provinsi') }}"
                   class="px-6 py-4 text-blue-900 font-semibold border-b-2 border-blue-900 whitespace-nowrap">
        ...
        <div class="bg-white shadow px-6 py-4">
            <div class="flex justify-between items-start flex-wrap gap-4">
                ...
                <form method="GET" class="flex items-start space-x-3 flex-wrap gap-3">
                    <label class="text-gray-700 pt-2 whitespace-nowrap">Filter berdasarkan Provinsi :</label>

                    <select name="provinsi" onchange="this.form.submit()"
                        class="border border-gray-300 rounded px-4 py-2 focus:ring-blue-500 min-w-[180px]">
                        <option value="all" {{ $provinsi == 'all' ? 'selected' : '' }}>Semua Provinsi</option>

                        @foreach($provinsiList as $item)
                            <option value="{{ $item }}" {{ $provinsi == $item ? 'selected' : '' }}>
                                {{ $item }}
                            </option>
                        @endforeach
                    </select>
                ...
                <table class="w-full">
                    ...
                    @forelse($sellers as $seller)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-gray-600">ID-Penjual-{{ $seller->id }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ $seller->propinsi }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ $seller->email_pic }}</td>
